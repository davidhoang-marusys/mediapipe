...
#ifndef MEDIAPIPE_CALCULATORS_UTIL_CONFIGURABLE_LANDMARKS_SMOOTHING_CALCULATOR_H_
#define MEDIAPIPE_CALCULATORS_UTIL_CONFIGURABLE_SMOOTHING_CALCULATOR_H_
...
//   }
//
class ConfigurableLandmarksSmoothingCalculator : public NodeIntf {
 public:
  ...
  static constexpr Output<mediapipe::LandmarkList>::Optional kOutLandmarks{
      "FILTERED_LANDMARKS"};
  static constexpr SideInput<float>::Optional kMinCutoff{"MIN_CUTOFF"};
  static constexpr SideInput<float>::Optional kBeta{"BETA"};
  MEDIAPIPE_NODE_INTERFACE(
        ConfigurableLandmarksSmoothingCalculator, kInNormLandmarks, kInLandmarks,
        kImageSize, kObjectScaleRoi, kOutNormLandmarks, kOutLandmarks,
        kMinCutoff, kBeta);
  ...
  static absl::Status UpdateContract(CalculatorContract* cc) {
    ...
    if (kMinCutoff(cc).IsConnected()) {
        RET_CHECK(kBeta(cc).IsConnected()) << "BETA side packet required when MIN_CUTOFF is used.";
    }
