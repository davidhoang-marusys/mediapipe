...
namespace mediapipe
{
  ...
  namespace
  {
    ...
    float dst_corners[8] = {0.0f,      dst_height,
        ...
        // Step 1: Smart downsampling with pyrDown before warp
        cv::Mat downsampled = *src;
        while (downsampled.cols / 2 >= output_width &&
               downsampled.rows / 2 >= output_height)
        {
          cv::Mat temp;
          cv::pyrDown(downsampled, temp);
          downsampled = std::move(temp);
        }

        // Step 2: Recompute source points based on new downsampled size
        float scale_x = static_cast<float>(downsampled.cols) / input.width();
        float scale_y = static_cast<float>(downsampled.rows) / input.height();

        cv::Mat scaled_src_points;
        src_points.copyTo(scaled_src_points);
        for (int i = 0; i < 4; ++i)
        {
          scaled_src_points.at<cv::Point2f>(i).x *= scale_x;
          scaled_src_points.at<cv::Point2f>(i).y *= scale_y;
        }

        // Step 3: Perspective transform
        cv::Mat dst_points = cv::Mat(4, 2, CV_32F, dst_corners);
        cv::Mat projection_matrix =
            cv::getPerspectiveTransform(scaled_src_points, dst_points);
        cv::Mat transformed;
        cv::warpPerspective(downsampled, transformed, projection_matrix,
